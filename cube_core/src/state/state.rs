...
pub const PERM_MOD: [usize; 7] = [8, 12, 6, 6, 24, 6, 6];
...
pub const NUM_PER_UNIQUE: [usize; 7] = [1, 1, 1, 4, 1, 4, 4];
...
impl CubeState {
    ...
    pub fn new(dimension: usize) -> Self {
        ...
        let mut perm = vec![0; orbits.iter().sum()];
    ...
    pub fn try_from_vec(vec: (Vec<Vec<u8>>, Vec<Vec<u8>>)) -> Result<Self> {
        let ori: Vec<u32> = vec.1.into_iter().map(|v| u32::from_vec(&v)).collect();
        let state = Self {
            perm: vec.0.into_iter().map(|v| u128::from_vec(&v)).collect(),
            ori: [ori[0], ori[1]],
        };
        state.check()?;
        Ok(state)
    ...
    pub fn cycle_decomposition(&self) -> Vec<Vec<Vec<usize>>> {
        (0..self.perm.len()).map(|i| self.cycle_decomposition_of(i)).collect()
    }
