...
fn test_new(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("n4 new from dimension");
    group.warm_up_time(std::time::Duration::from_secs(10));
    group.measurement_time(std::time::Duration::from_secs(30));
    group.sample_size(200);
    for dimension in dimensions {
        let layout = state::Layout::new(dimension as usize);
        group.bench_function(format!("dimensions {}", dimension), |f| {
            f.iter(|| {
                black_box(state::CubeState::with_layout(&layout));
            });
        });
    }
...
fn test_n_to_vect(c: &mut Criterion) {
    let dimensions = [2, 3, 4, 100, 1_000];
    let mut group = c.benchmark_group("n4 new to vect dimension");
    group.warm_up_time(std::time::Duration::from_secs(10));
    group.measurement_time(std::time::Duration::from_secs(30));
    group.sample_size(200);
    for dimension in dimensions {
        let layout = state::Layout::new(dimension);
        let cube = state::CubeState::with_layout(&layout);
        group.bench_function(format!("dimensions {}", dimension), |f| {
            f.iter(|| {
...
fn test_n_sum(c: &mut Criterion) {
    let dimensions = [2, 3, 4, 100, 1_000];
    let mut group = c.benchmark_group("n4 new test sum");
    group.warm_up_time(std::time::Duration::from_secs(10));
    group.measurement_time(std::time::Duration::from_secs(30));
    group.sample_size(200);
    for dimension in dimensions {
        let layout = state::Layout::new(dimension);
        let mut c1 = state::CubeState::with_layout(&layout);
        let c2 = state::CubeState::with_layout(&layout);
        group.bench_function(format!("dimensions {}, assigned", dimension), |f| {
            f.iter(|| {
                black_box(c1 += &c2);
            });
...
fn test_n_vect_into_state(c: &mut Criterion) {
    let dimensions = [2, 3, 4, 100, 1_000];
    let mut group = c.benchmark_group("n4 new Vect into State from dimension");
    group.warm_up_time(std::time::Duration::from_secs(10));
    group.measurement_time(std::time::Duration::from_secs(30));
    group.sample_size(200);
    for dimension in dimensions {
        ...
        group.bench_function(format!("dimensions {}", dimension), |f| {
            f.iter(|| {
                let _: state::CubeState = black_box(state::CubeState::from_vec(vect.clone(), &layout));
            });
        });
    }
}
...
//     group.finish();
// }
criterion_group!(benches, test_n_sum);
